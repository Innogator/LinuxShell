...
{
  //char cmdStr[81], *user, hostname[81];
  const char* whitespace = " \n\r\f\t\v"; /* Whitespace characters */
  char buffer[81]; /* Buffer to hold the command typed in 80 chars plus null terminator */
  char* token; /* Pointer to a token returned by strtok() */
  char* user;
  char* cwd;
  //memset(hostname, 0x00, sizeof(hostname));
  //gethostname(hostname, sizeof(hostname));
  user = (char *)getenv("USER"); /* Get the username of the user */
  cwd = (char *)getcwd("CWD"); /* Get the current working directory */
  if ( user == NULL)
    {
      strcpy(user, "unknown");
    }
